...
use sp1_sdk::{
    ...
    Elf, NetworkProver, ProveRequest, Prover, ProvingKey, SP1Proof, SP1ProofMode,
...
impl<H: OPSuccinctHost> OPSuccinctProofRequester<H> {
    ...
    pub async fn agg_proof_witnessgen(
        ...
        let stdin = get_agg_proof_stdin(
            ...
            boot_infos,
            headers,
            &self.program_config.range_pk.verifying_key(),
    ...
    pub async fn generate_mock_range_proof(
        ...
        let (pv, report) =
            match network_prover.execute(Elf::Static(get_range_elf_embedded()), stdin).await {
    ...
    pub async fn generate_mock_agg_proof(
        ...
        Ok(SP1ProofWithPublicValues::create_mock_proof(
            self.program_config.agg_pk.verifying_key(),
            pv.clone(),
            self.agg_mode,
            SP1_CIRCUIT_VERSION,
        ))
