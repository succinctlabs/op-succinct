...
use sqlx::{
    ...
    Acquire, Error, Postgres,
};

use crate::{CommitmentConfig, OPSuccinctRequest, RequestStatus, RequestType};

/// Adds a chain lock to the database.
pub async fn add_chain_lock<'a, A>(
    l1_chain_id: i64,
    l2_chain_id: i64,
    acquirable: A,
...
{
    let mut conn = acquirable.acquire().await?;
...
pub async fn is_chain_locked<'a, A>(
    l1_chain_id: i64,
    l2_chain_id: i64,
    interval: Duration,
    acquirable: A,
...
pub async fn update_chain_lock<'a, A>(
    l1_chain_id: i64,
    l2_chain_id: i64,
    acquirable: A,
) -> Result<PgQueryResult, Error>
...
pub async fn insert_request<'a, A>(
    req: &OPSuccinctRequest,
    acquirable: A,
) -> Result<PgQueryResult, Error>
where
    A: Acquire<'a, Database = Postgres>,
...
/// commitment.
pub async fn fetch_failed_request_count_by_block_range<'a, A>(
    start_block: i64,
    end_block: i64,
    l1_chain_id: i64,
    l2_chain_id: i64,
...
pub async fn fetch_highest_end_block_for_range_request<'a, A>(
...
pub async fn fetch_ranges_after_block<'a, A>(
    statuses: &[RequestStatus],
...
pub async fn fetch_request_count<'a, A>(
...
pub async fn fetch_requests_by_status<'a, A>(
    status: RequestStatus,
    commitment: &CommitmentConfig,
    l1_chain_id: i64,
    l2_chain_id: i64,
...
pub async fn fetch_all_requests_by_status<'a, A>(
    status: RequestStatus,
    from: Option<i64>,
    to: Option<i64>,
    acquirable: A,
...
pub async fn fetch_request<'a, A>(
    id: i64,
    acquirable: A,
    ...
    A: Acquire<'a, Database = Postgres>,
{
    ...
    let request = sqlx::query_as::<_, OPSuccinctRequest>("SELECT * FROM requests WHERE id = $1")
        .bind(id)
        .fetch_optional(&mut *conn)
        .await?;
    Ok(request)
...
pub async fn get_consecutive_complete_range_proofs<'a, A>(
    start_block: i64,
    end_block: i64,
    commitment: &CommitmentConfig,
    l1_chain_id: i64,
...
pub async fn fetch_failed_agg_request_with_checkpointed_block_hash<'a, A>(
    start_block: i64,
    end_block: i64,
    commitment: &CommitmentConfig,
    l1_chain_id: i64,
    ...
    A: Acquire<'a, Database = Postgres>,
{
    ...
    Ok(result
        .map(|r| (r.checkpointed_l1_block_hash.unwrap(), r.checkpointed_l1_block_number.unwrap())))
}
...
pub async fn fetch_active_agg_proofs_count<'a, A>(
    start_block: i64,
    commitment: &CommitmentConfig,
    l1_chain_id: i64,
    l2_chain_id: i64,
...
/// start block.
pub async fn fetch_unrequested_agg_proof<'a, A>(
    latest_contract_l2_block: i64,
    commitment: &CommitmentConfig,
    l1_chain_id: i64,
    l2_chain_id: i64,
...
/// latest_contract_l2_block.
pub async fn fetch_first_unrequested_range_proof<'a, A>(
    latest_contract_l2_block: i64,
    commitment: &CommitmentConfig,
    l1_chain_id: i64,
    l2_chain_id: i64,
...
pub async fn fetch_completed_ranges<'a, A>(
    commitment: &CommitmentConfig,
    latest_contract_l2_block: i64,
    l1_chain_id: i64,
    l2_chain_id: i64,
...
pub async fn update_l1_head_block_number<'a, A>(
    id: i64,
...
pub async fn update_prove_duration<'a, A>(id: i64, acquirable: A) -> Result<PgQueryResult, Error>
where
    A: Acquire<'a, Database = Postgres>,
{
    let mut conn = acquirable.acquire().await?;
...
pub async fn update_proof_to_complete<'a, A>(
    id: i64,
    proof: &[u8],
    acquirable: A,
) -> Result<PgQueryResult, Error>
...
pub async fn update_witnessgen_duration<'a, A>(
    id: i64,
    duration: i64,
    acquirable: A,
) -> Result<PgQueryResult, Error>
...
pub async fn update_request_status<'a, A>(
    id: i64,
...
pub async fn update_request_to_prove<'a, A>(
    id: i64,
    proof_id: B256,
    acquirable: A,
) -> Result<PgQueryResult, Error>
...
pub async fn update_request_to_relayed<'a, A>(
    id: i64,
    relay_tx_hash: B256,
...
pub async fn fetch_completed_agg_proof_after_block<'a, A>(
    latest_contract_l2_block: i64,
    commitment: &CommitmentConfig,
    l1_chain_id: i64,
    l2_chain_id: i64,
...
pub async fn insert_execution_statistics<'a, A>(
    id: i64,
...
pub async fn cancel_all_requests_with_statuses<'a, A>(
    statuses: &[RequestStatus],
    l1_chain_id: i64,
    l2_chain_id: i64,
    acquirable: A,
...
pub async fn delete_all_requests_with_statuses<'a, A>(
    statuses: &[RequestStatus],
    l1_chain_id: i64,
    l2_chain_id: i64,
    acquirable: A,
...
pub async fn cancel_prove_requests_with_different_commitment_config<'a, A>(
    commitment: &CommitmentConfig,
    l1_chain_id: i64,
    l2_chain_id: i64,
    acquirable: A,
...
pub fn insert_requests<'a, 'c, A>(
    requests: &'a [OPSuccinctRequest],
    acquirable: A,
) -> impl Future<Output = Result<PgQueryResult, Error>> + Send + 'a
where
