...
use sp1_sdk::{utils, Elf, ProveRequest, Prover, ProvingKey, ProverClient, SP1ProofMode};
use tracing::info;
...
async fn main() -> Result<()> {
    ...
    let network_prover =
        ProverClient::builder().network_for(network_mode).signer(network_signer.clone()).build().await;
