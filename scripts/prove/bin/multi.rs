...
use sp1_sdk::{utils, CpuProver, Elf, ProveRequest, Prover};
...
async fn main() -> Result<()> {
    ...
    if args.prove {
        ...
        let pk = prover.setup(Elf::Static(get_range_elf_embedded())).await.unwrap();
        // Generate proofs in compressed mode for aggregation verification.
        let proof = prover.prove(&pk, sp1_stdin.clone()).compressed().await.unwrap();
