...
async fn main() -> Result<()> {
    ...
    let pkey = prover.setup(Elf::Static(get_range_elf_embedded())).await?;
    ...
    if args.prove {
        prover.prove(&agg_pk, stdin).groth16().await.context("proving failed")?;
    } else {
        let (_, report) = prover.execute(Elf::Static(AGGREGATION_ELF), stdin).await?;
