...
use sp1_sdk::{
    utils, Elf, HashableKey, ProveRequest, Prover, ProverClient, ProvingKey, SP1Proof, SP1ProofWithPublicValues,
    SP1VerifyingKey,
};
use std::fs;
...
async fn main() -> Result<()> {
    ...
    let vkey = range_pk.verifying_key();

    let (proofs, boot_infos) = load_aggregation_proof_data(args.proofs, &prover, vkey).await;
    ...
    println!("Aggregate ELF Verification Key: {:?}", agg_pk.verifying_key().bytes32());

    if args.prove {
        prover.prove(&agg_pk, stdin.clone()).groth16().await.expect("proving failed");
    } else {
        let (_, report) = prover
