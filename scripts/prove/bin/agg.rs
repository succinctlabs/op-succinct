...
struct Args {
    ...
    /// Skip local CpuProver verification of range proofs. Useful when proofs were generated by the
    /// prover network, whose recursion pipeline may embed a different vk hash than what the local
    /// SDK computes.
    #[arg(long)]
...
async fn load_aggregation_proof_data(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let prover = if skip_verification {
            None
        } else {
            Some(blocking::CpuProver::new())
        };
...
async fn main() -> Result<()> {
    ...
    let (proofs, boot_infos) =
        load_aggregation_proof_data(args.proofs, &vkey, args.skip_verification).await;
