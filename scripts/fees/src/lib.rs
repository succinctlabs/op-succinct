...
impl fmt::Display for AggregateFeeData {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let eth = self.total_l1_fees / U256::from(10).pow(U256::from(18));
        let gwei = (self.total_l1_fees / U256::from(10).pow(U256::from(9)))
...
pub fn aggregate_fee_data(fee_data: Vec<FeeData>) -> Result<AggregateFeeData> {
    let mut aggregate_fee_data = AggregateFeeData {
        ...
        num_transactions: 0,
        total_l1_fees: U256::ZERO,
        total_fees: 0,
    };

    for data in fee_data {
        aggregate_fee_data.num_transactions += 1;
        aggregate_fee_data.total_l1_fees += data.l1_fee;
        aggregate_fee_data.total_fees += data.total_fee;
    }
