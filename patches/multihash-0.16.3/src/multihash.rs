...
impl<const S: usize> Multihash<S> {
        ...
        if !bytes.is_empty() {
            return Err(Error::InvalidSize(
                bytes
                    .len()
                    .try_into()
                    .expect("Currently the maximum size is 255, therefore always fits into usize"),
            ));
        }
    ...
    pub fn resize<const R: usize>(&self) -> Result<Multihash<R>, Error> {
        ...
        let mut mh = Multihash { code: self.code, size: self.size, digest: [0; R] };
        mh.digest[..size].copy_from_slice(&self.digest[..size]);
        Ok(mh)
    }

    /// Decomposes struct, useful when needing a `Sized` array or moving all the data into another
    /// type
    ///
...
impl<const S: usize> parity_scale_codec::Encode for Multihash<S> {
    fn encode_to<EncOut: parity_scale_codec::Output + ?Sized>(&self, dest: &mut EncOut) {
        ...
        //
        // We do not choose to encode &[u8] directly, because it will add extra bytes (the compact
        // length of digest). For a valid multihash, the length of digest must equal to
        // `size`. Therefore, we can only read raw bytes whose length is equal to `size`
        // when decoding.
...
mod tests {
    ...
    fn test_scale() {
        ...
        // println!("mh1: code = {}, size = {}, digest = {:?}", mh1.code(), mh1.size(),
        // mh1.digest());
        ...
        // println!("mh3: code = {}, size = {}, digest = {:?}", mh3.code(), mh3.size(),
        // mh3.digest());
