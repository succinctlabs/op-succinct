...
/// @custom:proxied true
/// @title OPSuccinctDisputeGameFactory
...
contract OPSuccinctDisputeGameFactory is OwnableUpgradeable, ISemver {
        ...
        returns (GameType gameType_, Timestamp timestamp_, IDisputeGame proxy_)
    {
        (
            ...
            Timestamp timestamp,
            address proxy
        ) = _disputeGameList[_index].unpack();
        (gameType_, timestamp_, proxy_) = (
            gameType,
            timestamp,
            IDisputeGame(proxy)
        );
    ...
    function create(
        ...
        proxy_ = IDisputeGame(
            address(impl).clone(
                abi.encodePacked(msg.sender, _rootClaim, parentHash, _extraData)
            )
        );
    ...
    function getGameUUID(
        ...
        uuid_ = Hash.wrap(
            keccak256(abi.encode(_gameType, _rootClaim, _extraData))
        );
    }
    ...
    function findLatestGames(
        ...
        for (uint256 i = _start; i >= 0 && i <= _start; ) {
            ...
            (GameType gameType, Timestamp timestamp, address proxy) = id
                .unpack();
