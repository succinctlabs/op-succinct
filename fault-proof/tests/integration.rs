...
mod integration {
    ...
    // Tests that prove-only mode correctly disables game creation, resolution, and bond claiming.
    // This mode is used during hardfork transitions when running old ELF versions alongside new ones.
    #[tokio::test(flavor = "multi_thread")]
    async fn test_prove_only_mode_skips_creation_resolution_claiming() -> Result<()> {
        info!("=== Test: Prove-Only Mode Skips Creation/Resolution/Claiming ===");

        let env = TestEnvironment::setup().await?;
        let factory = env.factory()?;
        let init_bond = factory.initBonds(TEST_GAME_TYPE).call().await?;

        // Advance time to ensure games won't be retired
        env.warp_time(10).await?;

        // === PHASE 1: Seed Games Manually ===
        info!("=== Phase 1: Seeding Initial Games ===");

        // Create a valid game before starting the prove-only proposer
        let block_1 = env.anvil.starting_l2_block_number + 10;
        let root_1 = env.compute_output_root_at_block(block_1).await?;
        env.create_game(root_1, block_1, u32::MAX, init_bond).await?;
        let (game_1_index, game_1_address) = env.last_game_info().await?;
        info!("✓ Created game 1 at index {} (address: {})", game_1_index, game_1_address);

        let initial_game_count = factory.gameCount().call().await?;
        info!("Initial game count: {}", initial_game_count);

        // === PHASE 2: Start Prove-Only Proposer ===
        info!("=== Phase 2: Starting Prove-Only Proposer ===");

        let proposer_handle = env.start_proposer_prove_only().await?;

        // Verify proposer is running
        assert!(!proposer_handle.is_finished(), "Proposer should be running");

        // === PHASE 3: Verify No Game Creation ===
        info!("=== Phase 3: Verify No Game Creation ===");

        // Wait for several fetch intervals (fetch_interval = 5s, so wait ~15s)
        tokio::time::sleep(Duration::from_secs(15)).await;

        let game_count_after_wait = factory.gameCount().call().await?;
        assert_eq!(
            game_count_after_wait, initial_game_count,
            "Prove-only proposer should NOT create new games. Expected: {}, Got: {}",
            initial_game_count, game_count_after_wait
        );
        info!("✓ No new games created by prove-only proposer");

        // === PHASE 4: Verify No Resolution ===
        info!("=== Phase 4: Verify No Resolution ===");

        // Warp time past challenge period to make game eligible for resolution
        env.warp_time(MAX_CHALLENGE_DURATION + 10).await?;
        info!("✓ Warped time past challenge duration");

        // Wait for proposer to potentially resolve (it shouldn't)
        tokio::time::sleep(Duration::from_secs(10)).await;

        // Verify game is still IN_PROGRESS (not resolved by prove-only proposer)
        let game_1 = env.fault_dispute_game(game_1_address).await?;
        let game_1_status_raw = game_1.status().call().await?;
        let game_1_status = GameStatus::try_from(game_1_status_raw).with_context(|| {
            format!("Failed to decode game status for {game_1_address}")
        })?;
        assert_eq!(
            game_1_status,
            GameStatus::IN_PROGRESS,
            "Prove-only proposer should NOT resolve games"
        );
        info!("✓ Game not resolved by prove-only proposer (still IN_PROGRESS)");

        // Manually resolve game for the next phase
        env.resolve_game(game_1_address).await?;
        info!("✓ Manually resolved game for bond claim test");

        // === PHASE 5: Verify No Bond Claiming ===
        info!("=== Phase 5: Verify No Bond Claiming ===");

        // Warp past finality delay to make bonds claimable
        env.warp_time(DISPUTE_GAME_FINALITY_DELAY_SECONDS + 10).await?;
        info!("✓ Warped time past finality delay");

        // Wait for proposer to potentially claim bonds (it shouldn't)
        tokio::time::sleep(Duration::from_secs(10)).await;

        // Verify credits still exist (not claimed by prove-only proposer)
        let credit = env.get_credit(game_1_address, PROPOSER_ADDRESS).await?;
        assert!(
            credit > U256::ZERO,
            "Prove-only proposer should NOT claim bonds. Credit should still exist."
        );
        info!("✓ Bonds not claimed by prove-only proposer (credit: {})", credit);

        // Verify proposer is still running (didn't crash)
        assert!(!proposer_handle.is_finished(), "Proposer should still be running");
        info!("✓ Prove-only proposer is still running normally");

        env.stop_proposer(proposer_handle);

        info!("=== Test Complete ===");
        info!("✓ Prove-only mode correctly disabled: game creation, resolution, bond claiming");
