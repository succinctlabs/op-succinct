...
mod integration {
    ...
    async fn test_honest_proposer_native() -> Result<()> {
        ...
        let tracked_games = env.wait_and_track_games(3, 90).await?;
        ...
        let resolutions = env.wait_for_resolutions(&tracked_games, 90).await?;
        ...
        env.wait_for_bond_claims(&tracked_games, PROPOSER_ADDRESS, 90).await?;
    ...
    async fn test_honest_challenger_native() -> Result<()> {
        ...
        // === PHASE 2: Challenge Period ===
        info!("=== Phase 2: Challenge Period ===");
        env.wait_for_challenges(&invalid_games, 90).await?;
        ...
        env.wait_for_bond_claims(&tracked_games, CHALLENGER_ADDRESS, 90).await?;
    ...
    async fn test_game_chain_validation_challenged_parent() -> Result<()> {
        ...
        // Wait for challenge
        env.wait_for_challenges(&[parent_game_address], 90).await?;
    ...
    async fn test_proposer_recovery_after_canonical_head_invalidation() -> Result<()> {
        ...
        let resolutions = env.wait_for_resolutions(first_two_games, 90).await?;
        verify_all_resolved_correctly(&resolutions)?;
        ...
        env.wait_for_bond_claims(first_two_games, PROPOSER_ADDRESS, 90).await?;
