...
async fn request_agg_proof(
    ...
    let private_key = match env::var("SP1_PRIVATE_KEY") {
        Ok(key) => key,
        Err(e) => return Err(AppError(anyhow::anyhow!("Failed to get SP1_PRIVATE_KEY: {}", e))),
    };
    let rpc_url = match env::var("PROVER_NETWORK_RPC") {
        Ok(url) => url,
        Err(e) => return Err(AppError(anyhow::anyhow!("Failed to get PROVER_NETWORK_RPC: {}", e))),
    };
    ...
    let headers = match fetcher.get_header_preimages(&boot_infos, l1_head.into()).await {
        Ok(h) => h,
        Err(e) => {
            log::error!("Failed to get header preimages: {}", e);
            return Err(AppError(anyhow::anyhow!("Failed to get header preimages: {}", e)));
        }
    };
    ...
    let stdin = match get_agg_proof_stdin(proofs, boot_infos, headers, &state.range_vk, l1_head.into()) {
        ...
        Err(e) => {
            ...
            return Err(AppError(anyhow::anyhow!("Failed to get agg proof stdin: {}", e)));
        }
    };
