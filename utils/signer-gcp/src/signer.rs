...
use gcloud_sdk::{
    ...
    tonic::{self, Request},
    GoogleApi, GoogleAuthMiddleware, TokenSourceType, GCP_DEFAULT_SCOPES,
};
use k256::ecdsa;
...
pub async fn init_client(creds_json_hex: String) -> Result<Client, GcpSignerError> {
    let creds_json_bytes =
        hex::decode(&creds_json_hex).map_err(|e| GcpSignerError::HexDecodeError(e.to_string()))?;
