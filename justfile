...
upgrade-l2oo l1_rpc admin_pk etherscan_api="":
  #!/usr/bin/env bash

  CHAIN_ID=$(jq -r '.chainId' contracts/zkconfig.json)
  if [ "$CHAIN_ID" = "0" ] || [ -z "$CHAIN_ID" ]; then
    echo "are you sure you've filled out your zkconfig.json? chain id still equals 0"
    exit 1
  fi

  VERIFY=""
  ETHERSCAN_API="{{etherscan_api}}"
  if [ $ETHERSCAN_API != "" ]; then
    VERIFY="--verify --verifier etherscan --etherscan-api-key $ETHERSCAN_API"
  fi
